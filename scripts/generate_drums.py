#!/usr/bin/env python3
"""Render drum kits from JSON configuration."""
...
from drumforge.synthesis import render_instrument


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate drum samples from config")
...
def main() -> int:
    ...
    velocities = config.get("default_velocities", [50, 90, 110, 127])
    default_rounds = config.get("default_round_robins", 1)
    output_root = Path(config.get("output_root", "output/sfz/analog_kit/samples"))
    ...
    print(f"Finished. {len(generated_files)} files written to {output_root}.")
    return 0
